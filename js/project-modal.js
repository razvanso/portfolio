...
projects.forEach((element) => {
  const pictureSources = element.pictures.sources.map((pictureSource) => {
    return `<source media="${pictureSource.media}" srcset="${pictureSource.srcset}" />`;
  }).join('');

  const highlights = element.highlights.map((highlight) => {
    return `<div class="project-client">${highlight}</div>`;
  }).join('');
